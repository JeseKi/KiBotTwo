...
本节还会修改 `gazebo.launch.py`、`sim_with_bridge.launch.py`、`CMakeLists.txt` 和 `package.xml`。这些文件的最终内容必须与 `../evidence/reference-runtime.patch` 中的成品分支 patch 完全一致；不要只保留“看起来等价”的 launch 写法。
...
本节的增量代码片段用于解释每个 launch 参数和 include 的作用；完成后必须用 `final-runtime/` 下的同名 `.bak` 文件逐文件核对。
...
但是只在 `nav2.launch.py` 声明这个参数还不够。它必须一路传到真正执行 `gz sim` 的 `gazebo.launch.py`。
...
接着把原来的单个 `ExecuteProcess` 拆成两个分支：
...
这样 `run_on_start:=true` 才会真正变成 Gazebo 命令里的 `-r`。
...
为了让 `check_runtime_deps:=false` 真正生效，检查函数一开始要读取 launch 参数：
...
如果依赖失败，错误信息不要只写“缺少符号”。把安装命令也放进异常文本，用户才能直接修复环境：
...
最后，`src/kibot_one_sim/scripts/check_nav2_runtime_deps.sh` 已经存在。为了让安装后的 package share 也带上这个脚本，修改：
...
```cmake
install(DIRECTORY models worlds launch config scripts
  DESTINATION share/${PROJECT_NAME}
)
```
...
在进入运行验证前，先执行 `../evidence/usage.md` 中的 runtime patch 完全一致审计。这个审计通过后，再看 lifecycle 和目标执行；否则 runtime 状态和成品分支不具备可比性。
