...
当前路线刚完成导航接入阶段。`00-系统基线` 已完成源码静态调查；`01-SLAM-建图与定位` 已有 SLAM 启动与参数基础；`02-Nav2-导航接入` 已通过完整仿真验收：Nav2 lifecycle active、costmap 正常发布、`NavigateToPose` 返回成功，并确认 Gazebo `/odom` 随目标执行前进。
...
| `03-边界探索` | 未开始 | `02-Nav2-导航接入` | 未展开 | 解锁未知空间主动探索 |
