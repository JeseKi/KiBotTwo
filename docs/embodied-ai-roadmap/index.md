...
当前 part：`01-SLAM-建图与定位`

当前路线仍处在基础能力建设阶段。`00-系统基线` 已完成源码静态调查；`01-SLAM-建图与定位` 已有路线、验收和运行时检查材料，但还需要通过实际 ROS2 / Gazebo 运行确认 `/scan`、`/odom`、TF 和 `/map`。

本次重写只整理顶层 `index.md`。现有阶段目录仍保留旧版平铺文件结构，暂不迁移为 `roadmap/`、`reference/`、`checklist/`、`evidence/` 四类子目录。
...
4. 进入 `01-SLAM-建图与定位/roadmap.md`，按阶段 01 的顺序推进。
5. 实施阶段 01 前，使用 `01-SLAM-建图与定位/runtime-checklist.md` 记录运行时事实。
6. 阶段 01 完成后，用 `01-SLAM-建图与定位/acceptance.md` 判断是否可以进入 `02-Nav2-导航接入`。
...
| `01-SLAM-建图与定位` | 进行中 | `00-系统基线` | 已存在 | 解锁 Nav2、后续探索和全局坐标中的旗帜估计 |
| `02-Nav2-导航接入` | 阻塞 | `01-SLAM-建图与定位` | 未展开 | 解锁点到点导航和探索目标执行 |
| `03-边界探索` | 阻塞 | `02-Nav2-导航接入` | 未展开 | 解锁未知空间主动探索 |
| `04-视觉旗帜检测` | 未开始 | `00-系统基线` | 未展开 | 解锁不依赖 `/flag_pose` 的旗帜发现事件 |
| `05-旗帜位置估计` | 阻塞 | `04-视觉旗帜检测` | 未展开 | 解锁发现后靠近旗帜或转换到地图坐标系 |
| `06-任务状态机` | 阻塞 | `03-边界探索`、`04-视觉旗帜检测` | 未展开 | 解锁探索、发现、靠近、停止的端到端任务编排 |
