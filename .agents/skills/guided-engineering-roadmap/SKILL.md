...
10. 当前 part 的文档完成后，默认执行“文档复测闭环”：让一个独立复测者只按刚写好的文档在新分支或新 worktree 中实际编辑代码、构建和验证；如果存在成品分支或探针分支，必须在 `reference/final-runtime/` 保存非 `docs/` diff 涉及的最终文件 `.bak` 副本，且复测产出的非 `docs/` diff 必须与成品分支相对同一 baseline 的非 `docs/` diff 完全一致。
