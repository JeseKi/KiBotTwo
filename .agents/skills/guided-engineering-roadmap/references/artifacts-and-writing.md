...
- 文档阅读顺序服从认知依赖；最终代码顺序服从工程组织。
- 当上层函数、节点回调、入口函数或配置段依赖多个 helper 时，不要先贴一大段包含未解释 helper 的调用方，再把 helper 放到后面补齐。
- 章节应先让读者知道“有哪些零件、各自负责什么、它们怎么连起来”，再进入代码片段。
- 代码片段不必机械地按最终文件从上到下追加；可以用明确锚点说明“插入到某段上方/下方”或“回到某个区域补充”。
- 不能为了文档顺序把最终代码组织改成不自然的 helper-first 堆叠；最终文件仍应符合项目语言和已有代码风格。

推荐使用“最终文件骨架 + 锚点插入”：

1. 先创建文件骨架，包含 imports、核心类型、以及必要的区域锚点，例如 `Public API`、`Internal helpers`、`Entry point`。
2. 用一张简短调用图或职责表说明 public 函数和 helper 的关系。
3. 先实现读者理解 public 行为所需的底层 helper 或小型数据结构。
4. 再回到 `Public API` 锚点，插入调用这些 helper 的上层函数。
5. 最后补入口、注册、测试或配置，并说明它们如何验证前面的行为。

这种写法要保证读者看到上层调用方时，已经理解被调用 helper 的职责；同时最终文件仍保持 public API 易发现、内部 helper 集中、入口函数位置自然。
