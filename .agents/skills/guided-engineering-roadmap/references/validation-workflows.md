...
`diff` 有任何输出都表示复测未通过。不要只比较文件名；必须比较完整 patch 内容。
...
验证完成后，报告相对 baseline 的非 docs diff；主 Agent 会用它和成品分支 diff 做完全一致审计。
验证后清理残留进程。
...
- 如果是非 `docs/` diff 与成品分支不一致，必须修正文档并从干净 baseline 重做；不能把差异记录为“可接受”后通过。
